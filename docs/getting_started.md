...
2. Preflight scheduler fields and config validity:
   - `python tools/hpc/preflight_scheduler_configs.py --fail-if-unready configs/stage1_y_host_validation_v1.yaml`
3. (Optional) patch scheduler placeholders in-place:
   - `python tools/hpc/preflight_scheduler_configs.py --set-partition <partition> --set-account <account> --write configs/stage1_y_host_validation_v1.yaml`
4. Validate config:
   - `python tools/validate_config.py configs/stage1_y_host_validation_v1.yaml`
5. Initialize run folder:
   - `python tools/init_run.py configs/stage1_y_host_validation_v1.yaml`
6. Place inputs in `results/runs/<run_id>/inputs/`:
   - `POSCAR`, `KPOINTS`, `INCAR`, `POTCAR`
7. Preflight required input decks:
   - `python tools/hpc/preflight_input_decks.py --require-potcar true --fail-if-unready configs/stage1_y_host_validation_v1.yaml`
8. Frontend dry-run (no submission):
   - `tools/hpc/run_vasp_pipeline.sh --mode dryrun --config configs/stage1_y_host_validation_v1.yaml`
9. Frontend smoke checks:
...
11. Parse outputs after completion:
   - `python tools/extract_metrics.py --run-dir results/runs/<run_id>`
12. Open/refresh the run HTML summary:
...
- `python tools/hpc/preflight_scheduler_configs.py --fail-if-unready configs/generated/stage1_campaign_<timestamp>`
- `python tools/hpc/preflight_input_decks.py --require-potcar true --fail-if-unready configs/generated/stage1_campaign_<timestamp>`
